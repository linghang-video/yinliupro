...
<html>
	...
	<body>
		<div class="i-body">
			...
			<div
				style="position: relative;height: 35px;overflow: hidden;font-size:15px;color:white;margin:10px 0;background: #332121;">
				<img src="http://a.xqr888.cn/images/c1ddbc.jpg"
					style="height: 18px;position: absolute;top: 10px;left: 10px;">
				<marquee direction="up" scrolldelay="300" style="margin-left:40px;font-size:13px;margin-top:10px;">
					分享给好友，对方点击后才会增加观看次数哟！</marquee>
			</div>
		...
		<div class="prompt" style="color:white;display:block;font-size: 12px;margin-left:10px;margin-top: 5px;"
			id="Content">
		...
		<script>
			...
			var bs = '<?php echo $bs; ?>' ;
			var Location, Pic, Headline, Content, Astrict, Switch, Pattern, Group_no, Destoon_ad_place_1_pic,
				Destoon_ad_place_2_pic, Destoon_ad_place_1, Destoon_ad_place_2, Num;
			load()
			...
			function views() {
				$.get("https://share154.mark666.cn/Plust/Fun/Views.php", {
					vc_id: vc_id
				}, function(res) {
					var date = JSON.parse(res);
				});
			}
			var token = 'http://url.urlsuo.cn/location/index.php?vc_id=' + vc_id + '&bs=' + bs + '&ip=' + ip;
			function ip_address() {
				var ips = '<?php echo $ips; ?>';
				$.get("https://share154.mark666.cn/Plust/Fun/IP_address.php", {
					vc_id: vc_id,
					ip: ips
				}, function(res) {
					var date = JSON.parse(res);
